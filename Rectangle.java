...
public boolean contains(Point p1){
    return ((p1.getX()>this.pointbasgauche.getX())&&(p1.getX()<this.pointbasgauche.getX()+this.largeur)&&(p1.getY()>pointbasgauche.getY())&&(p1.getY()<pointbasgauche.getY()+this.hauteur));
}
...
//Lorsque plusieurs  rectangles partagent le meme point, lorsqu'on va translater le point, cela agira sur tous les rectangles partageant le point la.
...
public static Rectangle hull(Rectangle[] rec) {
    ...
    for (Rectangle r : rec) {
        ...
        if (x < minX) minX = x;
        if (y < minY) minY = y;
        if (droite > maxX) maxX = droite;
        if (haut > maxY) maxY = haut;
    }
